...
const Home = () => {
  ...
  const userOptions = Object.entries(users).map(user => 
    <option value={user[0]} key={user[0]}>{user[1].name}</option>
    ...
    <div className="container">
      <div className="card text-center mt-5 w-50  mx-auto">
        ...
        <div className="card-body">
          <h5 className="card-title fw-bold text-danger text-uppercase">Sign In</h5>
        ...
        <button className=" btn btn-dark w-100" onClick={()=>dispatch(login(selectedUser))}>Sign In</button>
        </div>
      </div>
    </div>
  );
